...
class DamageCalculatorGen1Test extends AnyFlatSpec with should.Matchers:
  ...
  "Damage Calculator" should " calculate damage of an Electric pokemon that attack with Electric move " +
    "a Grass-Poison pokemon with Electrified, Sunny and TrickRoom field effects" in:
    val movePower = 10
    val damage =
      calculate(
        electricMove,
